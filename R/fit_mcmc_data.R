prep_and_run_mcmc <- function(
	...
	if (!dir.exists(dest_mcmc)) {
		dir.create(dest_mcmc, recursive = TRUE, showWarnings = FALSE)
	}
	if (!dir.exists(dest_posterior)) {
		dir.create(dest_posterior, recursive = TRUE, showWarnings = FALSE)
	}
