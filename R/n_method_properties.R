...
n_method_properties <- function(df, n_properties, n, n_pp) {
  ...
  get_method_rel_freq <- function(prop_vec) {
    temp <- df |>
      ...
      ungroup() |>
      pivot_wider(names_from = m, values_from = method) |>
      select(starts_with("m_"))

    m <- colnames(temp)

    temp |> count(!!!syms(m)) |> mutate(prob = n / sum(n))
  }
  ...
  pairs_sample <- sample(
    1:nrow(two_method_prob),
    n_properties,
    prob = two_method_prob$prob,
    replace = TRUE
  )
  ...
  subset_area_n_method <- function(prop_vec) {
    df |>
      ...
      ungroup() |>
      pivot_wider(names_from = n, values_from = method)
  }
  ...
  areas <- 1:n_properties |>
    map(\(x) {
      get_area_n(n_method_areas, unname(unlist(slice(sample_n_method, x))))
    }) |>
    list_c()
  ...
  get_joint_return <- function(return_df, m) {
    ...
    rdf <- return_df |>
      filter(property %in% p, delta > 0)

    missing_m <- rep(FALSE, 2)
    if (nm >= 2) {
      missing_m[1] <- ifelse(all(is.na(rdf$method_2)), TRUE, FALSE)
    }
    if (nm >= 3) {
      missing_m[2] <- ifelse(all(is.na(rdf$method_3)), TRUE, FALSE)
    }
    if (nm >= 4) {
      missing_m[3] <- ifelse(all(is.na(rdf$method_3)), TRUE, FALSE)
    }
    if (nm >= 5) {
      missing_m[4] <- ifelse(all(is.na(rdf$method_3)), TRUE, FALSE)
    }
  ...
  get_sample_occasions_n <- function(joint_return_df, m, max_pp) {
    ...
    while (not_all_used) {
      ...
      occasions_pool <- occasions_possible[which(
        !occasions_possible %in% occasions_have
      )]

      if (length(occasions_pool) == 0) {
        effort_sample <- effort_sample |>
          slice(seq(2, nrow(effort_sample), by = 2))
        occasions_have <- effort_sample$sample_occasions
        occasions_pool <- occasions_possible[which(
          !occasions_possible %in% occasions_have
        )]
  ...
  for (i in seq_len(n_properties)) {
    ...
    testthat::expect(
      effort$sample_occasions[nrow(effort)] <= n_pp,
