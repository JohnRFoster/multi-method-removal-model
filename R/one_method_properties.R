...
one_method_properties <- function(df, n_properties, n_pp) {
  ...
  sample_one_method <- sample(
    one_method_prob$method,
    n_properties,
  ...
  start_pp <- function() {
    sample.int(round(n_pp * 0.8), n_properties, replace = TRUE)
  }
  start <- start_pp()
  ...
  get_sample_occasions <- function(return_df, m, s, max_pp) {
    ...
    while (end_pp <= max_pp) {
      interval <- return_df |>
        filter(method %in% m, first_occurance == 0) |>
        pull(delta) |>
        sample(1)
  ...
  for (i in seq_len(n_properties)) {
    sample_occasions <- get_sample_occasions(
      one_method_return,
      sample_one_method[i],
      start[i],
      n_pp
    ...
    n_reps <- get_reps(
      ...
      sample_one_method[i],
      length(sample_occasions)
    )

    effort <- tibble(
      sample_occasions = sample_occasions,
      n_reps = n_reps,
